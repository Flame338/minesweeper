 ...
 *
 * All flattened cell indices are row*COLUMNS+col. Each entry point re-reads
 * `grid` and re-runs its deduction from scratch; on an Inconsistent board the
 * safe/mine sets below are only partial, so consult SolverIsConsistent first.
 */
...
int SolverMines(const Cell grid[ROWS][COLUMNS], int out[ROWS * COLUMNS]);
